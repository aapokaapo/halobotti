...
class CustomMatch(SQLModel, table=True):
    ...
    players: List["CustomPlayer"] = Relationship(back_populates="custom_matches", link_model=LinkTable, sa_relationship_kwargs={"lazy": "selectin"},)
    is_valid: bool = False
...
class CustomPlayer(SQLModel, table=True):
    ...
    custom_matches: List[CustomMatch] = Relationship(back_populates="players", link_model=LinkTable, sa_relationship_kwargs={"lazy": "selectin"},)
    is_valid: bool = False
